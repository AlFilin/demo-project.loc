...
class OfferModelHandler extends ModelHandler
{
    ...
    protected function clearCachedList()
    {
        ...
        $arCouponGroupIDList = (array) DB::table('lovata_coupons_shopaholic_group_offer')->where('offer_id', $this->obElement->id)->pluck('group_id')->all();
    ...
    protected function clearCachedListByProduct($iProductID)
    {
        ...
        //Get coupon group list
        $arCouponGroupIDList = (array) DB::table('lovata_coupons_shopaholic_group_product')->where('product_id', $iProductID)->pluck('group_id')->all();
        if (empty($arCouponGroupIDList)) {
            return;
        }
