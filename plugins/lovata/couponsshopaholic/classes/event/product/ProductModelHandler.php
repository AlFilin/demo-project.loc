...
class ProductModelHandler extends ModelHandler
{
    ...
    protected function clearCachedList()
    {
        ...
        $arCouponGroupIDList = (array) DB::table('lovata_coupons_shopaholic_group_product')->where('product_id', $this->obElement->id)->pluck('group_id')->all();
    ...
    protected function clearProductListByBrandID($iBrandID)
    {
        ...
        //Get coupon group list
        $arCouponGroupIDList = (array) DB::table('lovata_coupons_shopaholic_group_brand')->where('brand_id', $iBrandID)->pluck('group_id')->all();
        if (empty($arCouponGroupIDList)) {
            return;
        }
    ...
    protected function clearProductListByCategory($obCategory)
    {
        ...
        //Get coupon group list
        $arCouponGroupIDList = (array) DB::table('lovata_coupons_shopaholic_group_category')->where('category_id', $obCategory->id)->pluck('group_id')->all();
        if (empty($arCouponGroupIDList)) {
            return;
        }
